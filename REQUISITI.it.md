...
| INT-11 | **WooCommerce**               | Integrazione via **API**, non plugin. Due istanze da mappare: eventi/vendite da palco e libri/marketing. 🔴 **Il 27/08/2026 la credenziale attesa cambia verso.** Poiché è WooCommerce a scrivere su Salesforce, è ROMI a dover fornire a Pienissimo **l'endpoint Salesforce e un token di header** (blocca i test del 31/08). Non è stato deciso se Salesforce continui a rileggere gli ordini via API REST di WooCommerce e quindi se le Consumer Key e Secret servano ancora. Le due istanze non sono state riprese. |  M   |  🔴   |
...
| INT-13 | WooCommerce                   | Sviluppi necessari — lato Salesforce: campo `WooCommerce_Product_Id__c` su Prodotto, campo `WooCommerce_Order_Id__c` su Opportunità, pulsante generatore del link, logica di lettura ordini. **Il 27/08/2026 supera la parte lato Pienissimo:** il componente lato cliente è un **plugin WooCommerce scritto e mantenuto da Sabatino Rinaldi** (v1.3, sempre attivo), non lo snippet PHP _must-use plugin_ specificato da ROMI. Il generatore del link **non ha più bisogno della selezione prodotto e quantità** — i carrelli sono funnel di Funnel Kit che contengono già il prodotto, quindi il link porta **solo l'ID dell'opportunità**. |  M   |  ✅   |
| INT-14 | WooCommerce                   | **Direzione dell'integrazione — CHIUSA il 27/08/2026: è WooCommerce a scrivere su Salesforce.** Più preciso della raccomandazione generica del webhook: **i webhook standard di WooCommerce sono stati valutati in diretta e scartati** (un solo argomento per webhook, nessuna selezione multipla, nessun controllo sul body, non possono portare la struttura del cliente). Il meccanismo è un **plugin WooCommerce personalizzato con action hook PHP sul cambio di stato dell'ordine**, che invia un unico JSON con ordine, cliente e righe d'ordine. |  M   |  ✅   |
...
|  9  | **Consumer Key e Secret WooCommerce** per entrambe le istanze — ⚠ il 27/08/2026 resta da decidere se Salesforce chiami WooCommerce | INT-11 | Sabatino              |
|  9b | **Endpoint Salesforce e token di header** per il plugin WooCommerce — blocca i test di integrazione del 31/08 | INT-11 | **Aurel Mrruku (ROMI)** |
...
| :-: | ------------------------------------------------- | ------ | ------------------------------------------------ |
| 15  | Direzione integrazione WooCommerce                | INT-14 | ✅ **Chiusa il 27/08: WooCommerce scrive tramite plugin con action hook PHP sullo stato ordine** |
...
| ORD-12 | **Corretto il 27/08/2026.** Un ordine WooCommerce arriva su Salesforce quando il suo stato diventa **IN LAVORAZIONE oppure COMPLETATO**, con qualsiasi metodo di pagamento — bonifico, carta o PayPal. Verificato in diretta sul plugin consegnato; supera il precedente "invisibile finché non è COMPLETATO". Per i bonifici l'amministrazione continua a cambiare lo stato a mano alla ricezione. Ogni riga che arriva da WooCommerce è quindi già pagata. |  M   |
