...
**Version:** 1.6 — draft for approval · **Date:** 25 September 2026

> **What changed in 1.6.** Changes the client agreed during UAT: the quote gains a **`Firmato`** state (the signed document is back from DocuSign), at which the order is generated (24/09); tranches can also be defined **when the bundle is created**, are proposed as editable defaults on the quote, and are used as-is by WooCommerce bundle orders (25/09); bundles are classified by **anno accademico** and **evento di origine**, with a **Presenza piattaforma** flag (25/09); the `Recall tutor` opportunity type becomes **WooCommerce**, with a mandatory Recall Tutor / Pack Tutor origin (25/09). Sent for signature as one change set at the close of UAT.
...
| DM-17 | **Tranche** (renaming of "rate") — custom object created on the **Quote**, after products are selected, through a guided action that asks which Quote Line Items to include and the planned payment due date — or **at bundle creation**, on the bundle's components (v1.6). Each selected line stores the tranche reference and date; both propagate to the corresponding Order Item.                                           |  M   |   ✅   |
...
| BUN-08 | Classification: **anno accademico** + **evento di origine** (the event where the bundle is proposed) + **bundle typology** + **Presenza piattaforma** flag (v1.6, replaces anno solare → evento). The edition of each ticket is still derived from the order date.                                |  M   |   ✅   |
...
| ID     | Requirement                                                                                                                                                                                                                                                                                                         | Pri. | Status |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--: | :----: |
| ORD-01 | After products are selected on the Quote, the user creates each tranche through a guided action, selecting its Quote Line Items and entering the planned payment due date. If the Quote uses a bundle that already carries tranches, they are proposed as editable defaults (v1.6).                                 |  M   |   ✅   |
| ORD-02 | The tranche reference and date propagate from Quote Line Item to Order Item; for a WooCommerce bundle order they come from the tranches defined on the bundle, and the WooCommerce price is ignored (v1.6). The order passes to Mexal **in full**, carrying both values **at line level**, not as a tranche object. |  M   |   ✅   |
...
| INT-16 | WooCommerce                   | **Security of the identifier in the link**: id in clear (simple but guessable) or a signed token. 🔴 _ROMI recommends the signed token._ 🔴 **2026-08-28 — answered in practice, against that recommendation:** the delivered plugin sends the **15-character opportunity id in clear**, and the payload envelope has no signature, nonce or send timestamp field at all. The header token ROMI owes is therefore the **entire authentication** of this integration. Kept open because nobody has accepted that trade aloud.                                                                                                                                                                                                                                                                                                                                                               |  S   |   🔴   |
...
| 17  | Security of the id in the checkout link            | INT-16 | Signed token — ⚠ delivered in clear, needs a decision                                   |
...
| Quote       | Bozza → In trattativa (5-day validity) → **In attesa di accettazione** → Accettato (accounting copy received) → **Firmato** (signed; the order is generated) · or Rifiutata |
...
**Order type:** STANDARD · BUNDLE · PLUS · **Opportunity type:** Standard · WooCommerce (origin: Recall Tutor · Pack Tutor) · Plus Attivazione/Rinnovo
...
| SAL-21 | Opportunity type: “Standard”, “WooCommerce” (mandatory origin Recall Tutor or Pack Tutor) and “Plus Attivazione/Rinnovo” (v1.6).                                                                                                                                                                                                                                                                                                                         |  S   |
...
| BIG-18 | Participant landing reached by a link carrying the Account ID; with several events the event is chosen first. **2026-09-15: superseded in practice by the build** — the link now carries a per-invitation opaque token instead of the Account ID, and one invitation covers exactly one event, so the event is chosen by which link is opened. To be confirmed; see OI-136.                                                                              |  M   |
