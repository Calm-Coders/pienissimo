...
## UAT Deploys And Checks

The following UAT deploys succeeded against `a.mrruku@pienissimo.uat`:

- `0AfMA00000Chfc50AB` - dry-run for token URL Apex.
- `0AfMA00000Chf4E0AR` - deployed field metadata plus `Full_Permission`.
- `0AfMA00000Chg8L0AR` - deployed token URL Apex.
- `0AfMA00000ChkIQ0AZ` - dry-run for Asset `Rinuncia` status and controller.
- `0AfMA00000ChfsF0AR` - deployed Asset `Rinuncia` status and controller.
- `0AfMA00000Chn1l0AB` - dry-run for LWC `Rinuncia` button.
- `0AfMA00000ChmVW0AZ` - deployed LWC `Rinuncia` button.
- `0AfMA00000Chjsc0AB` - dry-run after allowing `Rinuncia` on any visible ticket.
- `0AfMA00000ChhPO0AZ` - deployed the same `Rinuncia` availability change.
- `0AfMA00000ChmVX0AZ` - dry-run for deleted-Contact form reopen.
- `0AfMA00000ChjRD0AZ` - deployed deleted-Contact form reopen.
- `0AfMA00000ChjhL0AR` and `0AfMA00000Chp8P0AR` - temporary duplicate-error
  guard; superseded same day.
- `0AfMA00000ChpEr0AJ` - dry-run after removing the custom duplicate error.
- `0AfMA00000ChpGT0AZ` - deployed removal of the custom duplicate error.
- `0AfMA00000Chqdx0AB` - dry-run for Contact layout plus
  `Full_Permission`.
- `0AfMA00000Chq660AB` - deployed Contact layout plus `Full_Permission`.
- `0AfMA00000Chr290AB` - dry-run for defaulting
  `Contact.Contatto_Principale__c` to `true`.
- `0AfMA00000ChmCB0AZ` - deployed the `Contatto_Principale__c` default change.
- `0AfMA00000ChlrD0AR` - dry-run for narrowing
  `ContactTriggerHandler` Account email sync on Contact update.
- `0AfMA00000Chrbd0AB` - deployed the narrowed
  `ContactTriggerHandler` Account email sync to UAT.
- `0AfMA00000ChrgT0AR` - dry-run for the null-email guard on
  `ContactTriggerHandler`.
- `0AfMA00000ChrlJ0AR` - deployed the null-email guard on
  `ContactTriggerHandler` to UAT.

Anonymous Apex backfills/checks run in UAT:

- `scripts/apex/backfill-event-invitation-token-urls.apex` updated 5 existing
  invitations so missing tokens could be generated and URL refresh queued.
- `scripts/apex/refresh-event-invitation-token-urls.apex` refreshed 2 stale
  stored URLs from Account/Campaign parameters to token URLs.
- `scripts/apex/probe-duplicate-event-invitation.apex` was a rollbacked probe
  of duplicate behavior; no probe data was kept.

`npm run vault:check` passed after the markdown changes. It still reports the
existing non-blocking requirement trace warnings.
